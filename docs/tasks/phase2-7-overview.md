...
### Task 1: 목업 → 실제 API 연동
- [ ] 모든 페이지의 `// 추후 실제 API 연동` 주석 부분 실제 데이터로 교체
- [ ] 대시보드 — 통계/최근활동/알림 실제 데이터
- [ ] 프로젝트 목록 — 내 프로젝트 실제 데이터
- [ ] 칸반 보드 — 실제 칸반 데이터 + 드래그 시 API 호출
- [ ] 결재 목록/상세 — 실제 데이터 + 승인/반려 액션
- [ ] 알림 목록 — 실제 데이터 + 읽음 처리

### Task 2: WebSocket/STOMP 실시간 연동
- [ ] 알림 — Spring Boot `/ws` WebSocket 연결
- [ ] 칸반 보드 — Spring Boot `/ws-kanban` 실시간 동기화
- [ ] 채팅 — STOMP 메시지 송수신
